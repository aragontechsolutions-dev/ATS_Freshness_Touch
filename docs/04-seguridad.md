...
| 41  | **Inundar a un cliente con el mismo aviso**                     | Se comprueba ANTES de enviar, no solo al registrar: el barrido ve la misma reserva cada 15 min y sin esto serian decenas de correos | `notifications/notifications.service.ts`   |
| 42  | Agotar la cuota del proveedor de correo tras una parada larga   | Tope de 100 reservas por pasada; el resto espera a la siguiente                                                                     | `notifications/reminder-sweep.service.ts`  |
| 43  | Tumbar la API con un fallo al mandar un recordatorio            | El barrido lo dispara un temporizador sin nadie escuchando: nunca lanza, para que no haya un rechazo sin capturar                   | `notifications/reminder-sweep.service.ts`  |
...
| Cuatro barridos seguidos sobre la misma reserva      | Un solo correo de recordatorio                               |
| Barrido tras una caída de seis horas                 | Avisa de las pendientes, no de las citas ya ocurridas        |
